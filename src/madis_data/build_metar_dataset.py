...
'''
Build Meteorological Assimilation Data Ingest System (MADIS) METAR datasets.
...
preprocesses MADIS METAR files, filters observations spatially or by station
...
2) Download and preprocess MADIS METAR files. Existing files are reused unless
...
- Downloaded MADIS METAR files and their preprocessed counterparts. Original
...
Example usage:
    ...
    build-metar-dataset 2024 1 1 2024 1 31 CO /path/to/data -n 8
    build-metar-dataset 2024 6 1 2024 6 30 ERCOT /path/to/data --remove-original
    build-metar-dataset 2024 1 1 2024 12 31 SMPC2 /path/to/data -v
'''
...
def run_build(
    ...
    '''
    Download, filter, and assemble MADIS METAR data.
    ...
    spatial_identifier
        ...
        RTO/ISO code, or a MADIS METAR station identifier.
    data_dir
    ...
    remove_original
        If True, remove original downloaded MADIS METAR files after successful
...
def arg_parse(
    ...
    '''
    Parse command-line arguments for the METAR dataset builder.
    ...
    code_description = (
        'Download and preprocess MADIS METAR observations for a selected U.S. '
        'state, territory, the contiguous United States (CONUS), an RTO/ISO region, or one '
        'station identifier. The script optionally removes the original downloaded MADIS METAR data files, '
        ...
        "  - Individual station: provide a four-character ICAO station ID from UCAR's METAR station list:\n"
    ...
    parser.add_argument(
        ...
        help=(
            ...
            'code, or a MADIS METAR station identifier.'
        ),
    )
    parser.add_argument(
        'data_dir',
    ...
    parser.add_argument(
        '--remove-original',
        action='store_true',
        help='Remove downloaded original MADIS METAR files after successful preprocessing.',
    )
    parser.add_argument(
        '-r',
        '--refresh',
...
def main(argv: list[str] | None = None) -> None:
    '''
    Run the command-line METAR dataset-building workflow.
