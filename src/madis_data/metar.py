...
def metar_local_file_paths(data_dir: Path, urls: list[str]) -> list[Path]:
    ...
    # Create Metar file paths by prefixing each URL filename with
    # - the local data directory
    # - the 'metar_' prefix so they cannot collide with other MADIS products
    file_paths = [data_dir / f'metar_{PurePosixPath(urlparse(file_url).path).name}' for file_url in urls]
...
def metar_station_ids2strings(station_id: xr.DataArray) -> np.ndarray:
    ...
    encoded_ids = _pack_metar_station_ids(station_id)
...
def download_metar(
    ...
    file_urls = metar_urls(start_date, end_date)
    ...
    if not refresh:
        # Construct the expected local original MADIS METAR file paths.
        file_paths = metar_local_file_paths(data_dir, file_urls)

        if preprocess:
            # Construct the expected preprocessed paths.
            file_paths_preprocessed = metar_local_file_paths_preprocessed(file_paths)

            # Obtain the variables required in a complete preprocessed file.
            _, _, _, mm_vars_pp = metar_variables()
            ...
            # Reuse existing original MADIS METAR files when preprocessing is disabled.
        ...
        skip_files = metar_local_file_paths(data_dir, skip_urls)
        skip_files_preprocessed = metar_local_file_paths_preprocessed(skip_files)

        # Remove original MADIS METAR files whose valid preprocessed files are being reused.
        if preprocess and remove_original:
            for file_path in skip_files:
                _remove_metar_source_file(file_path)
    ...
    print('Downloading METAR data files ...')
    ...
    for ii in range(0, len(file_urls), n_jobs):
        ...
        file_paths_short_list = metar_local_file_paths(data_dir, file_urls_short_list)
        urls_to_download = [
            file_url
            ...
            with TemporaryDirectory(
                ...
                for file_url, file_path in zip(
                    ...
                    if temporary_file is None:
                        ...
                        warnings.warn(f'Skipping unavailable MADIS METAR file: {file_url}', stacklevel=1)
        ...
        if preprocess:
            file_paths_preprocessed_short_list = metar_local_file_paths_preprocessed(file_paths_short_list)

            dss = extract_metar_data(file_paths_short_list, n_jobs=n_jobs)
            try:
                if len(dss) < len(file_paths_short_list):
                    raise ValueError(
                        'The number of downloaded original MADIS METAR files is greater than the '
                        'number of resulting processed datasets.'
                    )
                if len(dss) > len(file_paths_short_list):
                    raise ValueError(
                        'The number of downloaded original MADIS METAR files is smaller than the '
                        'number of resulting processed datasets.'
                    )
                ...
                for ds, file_path_preprocessed, file_path in zip(
                    ...
                    if remove_original:
                        # Remove the original MADIS METAR file and any accompanying ETAG.
                        _remove_metar_source_file(file_path)
            finally:
            ...
            # Record the downloaded original MADIS METAR file paths.
...
def extract_metar_data(file_paths: list[Path], n_jobs: int = 1) -> list[xr.Dataset]:
    ...
    try:
        ...
        with ThreadPoolExecutor(max_workers=n_jobs) as executor:
            for ds in executor.map(extract_metar_data_single, file_paths):
                dss.append(ds)
        completed = True
    finally:
        if not completed:
...
def extract_station_data(
    ...
    station_id_keys = _prepare_metar_station_id_keys(ds['stationId'], requested_station_ids)
    ...
    if requested_station_ids:
        ...
        record_positions, station_codes = _select_metar_station_records(station_id_keys, requested_station_ids)
        del station_id_keys
...
def _bulk_frame_to_station_datasets(
    ...
    station_table_ids = np.asarray(metar_station_ids2strings(ds['stationID']))
...
def _pack_metar_station_ids(station_id: xr.DataArray) -> np.ndarray | None:
    '''
...
def _prepare_metar_station_id_keys(
    station_id: xr.DataArray,
    ...
    return np.asarray(metar_station_ids2strings(station_id))


def _remove_metar_source_file(file_path: Path) -> None:
    '''
    Remove one original METAR file and its ETag file.
    '''
...
def _select_metar_station_records(
